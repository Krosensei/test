...
if ($_REQUEST['act'] == 'view_install_log')
{
    ...
    if (empty($code) || file_exists(ROOT_PATH . DATA_DIR . '/integrate_' . $code . '_log.log' ))
    {
        sys_msg($_LANG['lost_intall_log'], 1);
    }

    include(ROOT_PATH . DATA_DIR . '/integrate_' . $code . '_log.log');
...
if ($_REQUEST['act'] == 'sync')
{
    ...
    /* 保存修改日志 */
    $fp = @fopen(ROOT_PATH . DATA_DIR . '/integrate_' . $_SESSION['code'] . '_log.log', 'wb');
    $log = '';
    if (isset($del_list))
    {
