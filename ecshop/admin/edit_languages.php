...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $file_name = basename($_REQUEST['file_path']);
    $file_name = explode('.', $file_name);
    //从languages目录下获取语言项文件
    $lang_arr    = get_lang_arr();
    // 不是languages目录下的语言项文件不能修改 --20191225   pangxp
    if (!isset($lang_arr[$file_name['0']])) 
        sys_msg($_LANG['unchangeable_file'], 0, $link);
    // 修改的内容不能含有特殊字符
    foreach ($_POST['item_id'] as $item) {
        if (preg_match('/[\/*.<>?()]+/', $item)) {
            sys_msg($_LANG['contains_special_characters'] . $item, 0, $link);
        }
    }
    foreach ($_POST['item_content'] as $item) {
        if (preg_match('/[\/*.$\<>?()]+/', $item)) {
            sys_msg($_LANG['contains_special_characters'] . $item, 0, $link);
        }
    }
...
function set_language_items($file_path, $src_items, $dst_items)
{
    ...
    for ($i = 0; $i < $snum; $i++)
    {
        // 不是$_LANG['\w+']格式的不能修改 --20191225   pangxp
        $temp = explode('=', $src_items[$i]);
        $temp = preg_replace('/\$_LANG\[\'(\w+)\'\]/', '$_LANG\[\'$1\'\]', $temp[0]);
        if (preg_match('/\\'.$temp.'/', $file_content)) {
 ...
 * 从languages目录下获取语言项文件
 * @access  public
 * @param   string      $lang_path     存放语言项列表的文件的绝对路径
 * @return  array       返回获取语言项文件
 */
function get_lang_arr($lang_path = '../languages/zh_cn')
{
    $lang_arr    = array();
